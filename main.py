...
def register():
    ...
    if form.validate_on_submit():
        ...
        user = user.scalar()
        if user :
            flash("This email is already registered, try to log in instead !")
            return redirect(url_for('login'))

        new_user = User(
        email=form.email.data,
        password=generate_password_hash(str(form.password.data),salt_length=8),
        name=form.name.data,
        )
...
def show_post(post_id):
    ...
    if form.validate_on_submit() :
        if current_user.is_authenticated :
            ...
            flash("You need to login to register comments ! ")
            return redirect(url_for('login'))
